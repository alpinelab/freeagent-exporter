class DashboardController < ApplicationController
  ...
  def index
    ...
    @authorize_url = client.auth_code.authorize_url(:redirect_uri => 'http://localhost:3000/oauth2/callback') if setting.nil?
  ...
  def callback
    setting = Setting.first
    token = oauth2_freeagent_client.auth_code.get_token(params['code'], :redirect_uri => 'http://localhost:3000/oauth2/callback')
