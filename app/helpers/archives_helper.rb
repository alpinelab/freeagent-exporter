module ArchivesHelper
  ...
  def download_button(archive)
    link_to(
      content_tag(:span, class: %w{glyphicon glyphicon-download-alt}).html_safe + "Download Archive",
      archive.s3_url,
  ...
  def transactions_left_to_explain_label(archive)
    content_tag(:span, "#{archive.transactions_left_to_explain} transactions left to explain", class: %w{label label-warning}).html_safe
  end

  def generate_button(archive)
    link_to(
      content_tag(:span, class: %w{glyphicon glyphicon-cog}).html_safe + "Generate Archive",
      archive,
      method: 'PUT',
